...
@Composable
fun ContenidoNfcEliminar(navController: NavHostController, servicio: NfcApiService, nfcId: Int) {
    ...
    if (showDialog) {
        AlertDialog(
            ...
            text = { Text("¿Está seguro de eliminar el NFC?") },
            confirmButton = {
                Button(
                    onClick = {
                        showDialog = false
    ...
    if (borrar) {
        LaunchedEffect(Unit) {
            servicio.deleteNfc(nfcId.toString())
            borrar = false
            navController.navigate("nfc")
        }
    }
